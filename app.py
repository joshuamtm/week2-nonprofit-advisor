...
with st.sidebar:
    ...
    if st.session_state.advising_started:
        ...
        if st.session_state.messages:
            ...
            profile = st.session_state.org_profile
            export_lines = [
                f"# Technology Advice for {profile.get('org_name', 'Your Organization')}",
                f"*Generated {datetime.now().strftime('%B %d, %Y')} by MTM Nonprofit Tech Advisor (AI)*",
                "",
                "## Organization Profile",
            ]
            ...
            for msg in st.session_state.messages:
                if msg["role"] == "user":
                    export_lines.append(f"## Question")
            ...
            export_lines.append("---")
            export_lines.append("*This advice was generated by an AI advisor from Meet the Moment (mtm.now).*")
            export_lines.append("*For professional consulting, contact joshua@mtm.now.*")
            ...
            st.download_button(
                ...
                data=export_text,
                file_name=f"mtm-advice-{profile.get('org_name', 'session').lower().replace(' ', '-')}.md",
                mime="text/markdown",
